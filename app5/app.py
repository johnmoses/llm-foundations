...
def chat():
    ...
    if intent == "get_stock_price":
        ...
        response = (
            ...
            if ticker
            else "Please specify a stock ticker symbol."
        )
        ...
        if len(numbers) >= 3:
            ...
            response = (
                "Please provide principal, rate, and time for interest calculation."
            )
    elif intent == "get_compliance_docs":
        topics = ["gdpr", "sox", "basel"]
        topic = next((t for t in topics if t in user_message.lower()), None)
        response = (
            get_compliance_docs(topic)
            if topic
            else "Please specify a compliance topic like GDPR, SOX, or Basel."
        )
    else:
